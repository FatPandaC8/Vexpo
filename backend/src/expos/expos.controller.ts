...
export class ExposController {
  ...
  @Roles('organizer')
  @Get(':id/booths/all')
  @ApiOperation({ summary: 'Get ALL booths of expo (organizer/admin — not filtered by status)' })
  async findAllBoothsManage(
    @Param('id', ParseIntPipe) id: number,
    @Req() req: any,
  ) {
    const roles: string[] = req.user.roles.map((r: string) => r.toLowerCase())
    if (roles.includes('admin')) return this.expoService.findAllBoothsByExpoId(id)
