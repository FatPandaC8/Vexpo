...
export interface OccupiedCell extends Cell { name?: string }
    ...
    <div class="bg-gray-50 border border-gray-400 rounded-2xl p-5 w-full">
      ...
      <div v-for="r in rows" :key="r" class="flex items-center gap-3 mb-1.5">
        ...
        <button
            ...
            :class="{
                ...
                'hover:bg-gray-100 border-gray-400 bg-white': !isSelected(r,c) && !isOccupied(r, c)
            }"
      ...
      <div class="flex items-center gap-4 mt-3 pt-3 border-t border-gray-200">
        <span class="flex items-center gap-1.5 text-[11px] text-gray-500">
          <span class="w-3.5 h-3.5 rounded bg-[#3d52d5]/10 border-2 border-[#3d52d5] block" />
          Your booth
        </span>
        <span class="flex items-center gap-1.5 text-[11px] text-gray-500">
          <span class="w-3.5 h-3.5 rounded bg-gray-200 border border-gray-300 block" />
          Taken
        </span>
        <span class="flex items-center gap-1.5 text-[11px] text-gray-500">
          <span class="w-3.5 h-3.5 rounded bg-white border border-dashed border-gray-300 block" />
          Free
        </span>
