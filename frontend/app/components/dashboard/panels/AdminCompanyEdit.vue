...
async function submit(event: FormSubmitEvent<typeof state>) {
  ...
  error.value   = null
  try {
    const updated = await api.patch<any>(`/companies/${props.company.id}`, event.data)
    success.value = true
  ...
  <div>
    <div class="flex items-center gap-3 mb-8">
      ...
      <div>
        ...
        <p class="text-sm text-gray-400 mt-0.5">{{ company.industry ?? 'Admin — edit company' }}</p>
      </div>
    </div>
    ...
    <Transition name="fade">
      <div v-if="success" class="mb-5 flex items-center gap-2 rounded-xl bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm text-emerald-700">
        <UIcon name="i-lucide-circle-check" class="shrink-0 text-emerald-500" />Company updated.
      </div>
    </Transition>
    <Transition name="fade">
      <div v-if="error" class="mb-5 flex items-center gap-2 rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">
        <UIcon name="i-lucide-circle-alert" class="shrink-0 text-red-500" />{{ error }}
      </div>
    </Transition>
    ...
    <div class="mt-10 pt-8 border-t border-gray-100">
      ...
      <Transition name="fade">
        <div v-if="showDelete" class="mt-4 p-5 rounded-xl border border-red-200 bg-red-50/30">
          <p class="text-sm text-red-700 mb-3">Type <strong>{{ company.name }}</strong> to confirm:</p>
          <UInput v-model="deleteConfirm" placeholder="Company name" class="mb-4 w-full max-w-xs" :ui="{ base: 'border border-red-200 focus:border-red-400 px-3 h-10 rounded-xl' }" />
          <UButton :disabled="!canDelete || deleteLoading" :loading="deleteLoading" size="sm" class="rounded-xl cursor-pointer px-5" :class="canDelete ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-400 cursor-not-allowed'" @click="deleteCompany">
