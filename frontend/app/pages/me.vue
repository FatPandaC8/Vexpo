...
const roleBadgeClass = computed(() => {
  if (auth.isVisitor.value)   return 'bg-blue-100 text-blue-700 border border-blue-200'
  if (auth.isExhibitor.value) return 'bg-violet-100 text-violet-700 border border-violet-200'
  if (auth.isOrganizer.value) return 'bg-emerald-100 text-emerald-700 border border-emerald-200'
  if (auth.isAdmin.value)     return 'bg-red-100 text-red-700 border border-red-200'
  ...
  <div class="min-h-screen bg-linear-to-r from-blue-50 to-blue-100">
    <div class="max-w-6xl mx-auto px-8 py-10">
      ...
      <div class="flex items-center gap-2 mb-10 text-sm">
        <NuxtLink to="/" class="text-gray-400 hover:text-gray-700 transition flex items-center gap-1.5">
      ...
      <div class="grid grid-cols-4 gap-6 items-start">
        ...
        <div class="col-span-1 space-y-4">
          ...
          <UCard class="rounded-2xl border border-gray-300 p-6 text-center bg-white">
            ...
            <p class="font-bold text-gray-900 text-sm mb-0.5 truncate">{{ auth.user.value?.name }}</p>
            <p class="text-xs text-gray-400 truncate mb-3">{{ auth.user.value?.email }}</p>
        ...
        <div class="col-span-3">
            ...
            <UCard class="rounded-2xl border border-gray-400 p-8 bg-white">
              ...
              <div class="flex items-center gap-3 mb-8">
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-400">Update your display name and email address</p>
                </div>
              </div>

              <Transition name="fade">
                <div v-if="profileSuccess" class="mb-6 flex items-center gap-2 rounded-xl bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm text-emerald-700">
                  <UIcon name="i-lucide-circle-check" class="shrink-0 text-emerald-500" />
                  Profile updated successfully.
                </div>
              </Transition>
              <Transition name="fade">
                <div v-if="profileError" class="mb-6 flex items-center gap-2 rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">
                  <UIcon name="i-lucide-circle-alert" class="shrink-0 text-red-500" />
                  {{ profileError }}
                </div>
              </Transition>

              <UForm :state="profileState" :schema="profileSchema" class="space-y-5" @submit="saveProfile">

                <UFormField name="name" label="Full Name" :ui="{ error: 'text-red-500 italic text-xs mt-1' }">
                  <UInput
                ...
                <UFormField name="email" label="Email Address" :ui="{ error: 'text-red-500 italic text-xs mt-1' }">
                  <UInput
                ...
                <div>
                  ...
                  <div class="flex items-center gap-3 h-10 px-3 rounded-xl border border-gray-100 bg-gray-50">
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold" :class="roleBadgeClass">
                      <UIcon :name="roleIcon" class="w-3 h-3" />
                      {{ roleLabel }}
                    </span>
                    <span class="text-xs text-gray-400">Contact admin to change role</span>
                  </div>
                </div>
          ...
          <template v-else-if="activeSection === 'security'">
            ...
            <UCard class="rounded-2xl border border-gray-400 p-8 bg-white">
              ...
              <Transition name="fade">
                <div v-if="passwordSuccess" class="mb-6 flex items-center gap-2 rounded-xl bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm text-emerald-700">
                  <UIcon name="i-lucide-circle-check" class="shrink-0 text-emerald-500" />
                  Password updated successfully.
                </div>
              </Transition>
              <Transition name="fade">
                <div v-if="passwordError" class="mb-6 flex items-center gap-2 rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">
                  <UIcon name="i-lucide-circle-alert" class="shrink-0 text-red-500" />
                  {{ passwordError }}
                </div>
              </Transition>

              <UForm :state="passwordState" :schema="passwordSchema" class="space-y-5" @submit="savePassword">

                <UFormField name="currentPassword" label="Current Password" :ui="{ error: 'text-red-500 italic text-xs mt-1' }">
                  <UInput
                ...
                <UFormField name="newPassword" label="New Password" :ui="{ error: 'text-red-500 italic text-xs mt-1' }">
                  <UInput
                ...
                <UFormField name="confirmPassword" label="Confirm New Password" :ui="{ error: 'text-red-500 italic text-xs mt-1' }">
                  <UInput
